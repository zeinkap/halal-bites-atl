...
test.describe('Restaurant Management', () => {
  ...
  test('should allow adding a new restaurant with all details', async ({ page, request }) => {
    ...
    const verificationCheckbox = page.locator('[data-testid="halal-verification-checkbox"]');
    await verificationCheckbox.check();
    
    // Verify submit button is enabled
    const submitButton = page.locator('[data-testid="submit-restaurant-button"]');
    ...
    if (!response.ok()) {
      ...
      throw new Error(`Failed to create restaurant: ${errorData.error || response.statusText()}`);
    }

    // Verify restaurant in database
    const savedRestaurant = await waitForRestaurantInDB(request, TEST_RESTAURANTS.BASIC.name);
  ...
  test('should prevent duplicate restaurant names', async ({ page, request }) => {
    // Create first restaurant via API
    const firstRestaurant = await createRestaurantViaAPI(request, TEST_RESTAURANTS.DUPLICATE);
    expect(firstRestaurant).toBeDefined();
    
    // Verify first restaurant was created
    const savedRestaurant = await waitForRestaurantInDB(request, TEST_RESTAURANTS.DUPLICATE.name);
    expect(savedRestaurant).toBeDefined();
    ...
    const errorData = await response.json();
    expect(errorData.error).toContain('Unique constraint failed');
    expect(errorData.error).toContain('name');
    
    // Verify only one instance exists
    const getResponse = await request.get('/api/restaurants');
    const restaurants = await getResponse.json() as Restaurant[];
    const duplicates = restaurants.filter((r) => r.name === TEST_RESTAURANTS.DUPLICATE.name);
